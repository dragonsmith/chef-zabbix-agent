...
cron 'write zabbix_key with unixtime to redis' do
  minute '*/1'
  command "/usr/bin/redis-cli -a #{get_redis_password(node['zabbix']['redis_config'])} -h #{get_redis_host(node['zabbix']['redis_config'])} -p #{get_redis_port(node['zabbix']['redis_config'])} set zabbix_key `date +\\%s` > /dev/null"
end
