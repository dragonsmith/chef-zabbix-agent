...
"/usr/bin/psql -h 127.0.0.1 -p [0-9]* -U postgres -d postgres -A -t -c SELECT round(blks_hit*100/(blks_hit+blks_read)\\, 2) AS cache_hit_ratio FROM pg_stat_database WHERE datname \\= '*' and blks_read > 0 union all select 0.00 AS cache_hit_ratio order by cache_hit_ratio desc limit 1",
...
"/usr/bin/psql -h 127.0.0.1 -p [0-9]* -U postgres -d postgres -A -t -c select count(*) from pg_stat_activity where state \\= 'active' and now() - query_start > '[0-9]* sec'\\:\\:interval and query ~* '^(insert|update|delete)'",
"/usr/bin/psql -h 127.0.0.1 -p [0-9]* -U postgres -d postgres -A -t -c select count(*) from pg_stat_activity where state \\= 'active' and now() - query_start > '[0-9]* sec'\\:\\:interval",
